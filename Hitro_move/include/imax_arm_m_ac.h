...
{
public:
  imax_arm_m_ac(int motor_number){
        sub_ps5= nh.subscribe("/joy_command_arm", 1,&imax_arm_m_ac::joy2xyz,this);
        // sub_button=nh.subscribe("/cmd_button", 1,&imax_arm_m_ac::msg_button,this);
        // sub_ps5= nh.subscribe("/cmd_vel", 1,&imax_arm_m::urdf_check,this);
    ...
    // else{
        ...
        joint_rad_urdf.name[0]="turn_table";
        joint_rad_urdf.name[1]="first_link";
        joint_rad_urdf.name[2]="second_link";
        joint_rad_urdf.name[3]="hand_pitch";
        joint_rad_urdf.name[4]="hand_roll";
        joint_rad_urdf.name[5]="hand_yaw";
        joint_rad_urdf.name[6]="hand_con_right";
        joint_rad_urdf.name[7]="hand_con_left";
        
    }
    ~imax_arm_m_ac(){}
    void joy2xyz(const arm_msgs::ArmCommand& msg);//ps5メッセージを受け取り、xyzを変化
    void kinetics();//運動学
    ...
    void urdf_check(const arm_msgs::ArmCommand& msg);
    void start();
    void msg_button(const arm_msgs::ArmCommand& msg);
