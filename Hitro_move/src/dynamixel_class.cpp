...
MultiDynamixelControl::MultiDynamixelControl(int dxl_quantity){
  ...
  max_angle_deg.data.resize(Dynamixel_Quantity);
  min_angle_deg.data.resize(Dynamixel_Quantity);
  motor_direction.data.resize(Dynamixel_Quantity);
  //fl,fr,bl,br
  max_angle_deg.data={40,90,90,40,90,90,40,90,90,40,90,90};
  min_angle_deg.data={-40,-90,-270,-40,-90,-270,-40,-90,-270,-40,-90,-270};
  motor_direction.data={2,-2,2, 2,2,-2, -2,-2,2, -2,2,-2};
 ...
 timer_jointstate=node.createTimer(ros::Duration(0.1),&MultiDynamixelControl::TimerCallbackJointState,this);
 timer_goalposition=node.createTimer(ros::Duration(0.005),&MultiDynamixelControl::TimerCallbackGoalPosition,this);
/*
ros::NodeHandle pnh("~");
...
void MultiDynamixelControl::CallbackTargetJointState_(const sensor_msgs::JointState& target_msg_jointstate){
  for(int i=0;i<(int)target_msg_jointstate.name.size();i++){
    ...
    if(itr!=joint_name.end()){
        int target_joint_number=itr->second;
        if((target_joint_number+1)%3!=0){ //joint3ではないか
            target_rad.data[target_joint_number]=target_msg_jointstate.position[i];
        }
        else{
          target_rad.data[target_joint_number]=target_msg_jointstate.position[i]+target_rad.data[target_joint_number-1];
        }
...
void MultiDynamixelControl::TimerCallbackJointState(const ros::TimerEvent &a){
  PublishJointState();
}
void MultiDynamixelControl::TimerCallbackGoalPosition(const ros::TimerEvent &a){
  MoveTargetPosition();
}
...
void MultiDynamixelControl::PublishJointState(){
  ...
  for(int i=0;i<Dynamixel_Quantity;i++){
    if(ping_result[i]==true){
    if((i+1)%3!=0){   
        jointstate.position.at(i)=(now_rad.data[i]/2)-initial_rad.data[i];
    }
    else{//joint3
        jointstate.position.at(i)=(now_rad.data[i]/2)+jointstate.position.at(i)-initial_rad.data[i];
    }
    }
  }
...
void MultiDynamixelControl::MoveTargetPositionSingle(int i){
    if(ping_result[i]==true){
      ...
      //now_rad.data[i]=singledynamixel.at(i).getposition();
     // ROS_INFO("%d->%lf",singledynamixel.at(i).dxl_id,target_rad.data[i]);
      if(target_rad.data[i]<deg2rad(max_angle_deg.data[i]) && target_rad.data[i]>deg2rad(min_angle_deg.data[i])){
        control_dxl_rad.data[i]=motor_direction.data[i]*target_rad.data[i]+initial_rad.data[i];
       singledynamixel.at(i).dxl_wb.goalPosition(singledynamixel.at(i).dxl_id,control_dxl_rad.data[i],&singledynamixel.at(i).log);
       ROS_INFO("%s(%d)->target:%lf actual:%lf",singledynamixel.at(i).dxl_name_,singledynamixel.at(i).dxl_id,target_rad.data[i],control_dxl_rad.data[i]);
      }
      else{ROS_WARN("%d->%lf",singledynamixel.at(i).dxl_id,target_rad.data[i]);}
    }
}
