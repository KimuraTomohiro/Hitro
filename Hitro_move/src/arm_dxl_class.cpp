#include "arm_dynamixel_hedder.h"
...
bool MultiDynamixelControl::ConnectAll(){
  ...
  for(int i=0;i<Dynamixel_Quantity;i++){
    ...
    if(singledynamixel.at(i).ping()==false){
      ...
      // std::cout<<initial_rad.data[i]<<std::endl;
      ping_result[i]=true;
...
bool MultiDynamixelControl::SetupPubSub(){
  ...
  // pub_velocitystatus=node.advertise<Hitro_move::wheelstatus>("present_velocity_status",1);
  // pub_velocitytarget=node.advertise<Hitro_move::wheelstatus>("target_velocity",1);
