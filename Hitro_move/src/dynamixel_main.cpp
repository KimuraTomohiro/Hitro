...
port_name.resize((int)member_list.size());
  ...
  for (int32_t i = 0; i < member_list.size(); ++i)
  {
    ...
    if (member_list[i]["p_gain"].getType() == XmlRpc::XmlRpcValue::TypeInt)
      ...
      i_gain = static_cast<int>(member_list[i]["i_gain"]);
          if (member_list[i]["d_gain"].getType() == XmlRpc::XmlRpcValue::TypeInt)
      d_gain = static_cast<int>(member_list[i]["d_gain"]);
    ...
    ROS_INFO("[%i] id: %i, dxl_name: %s, port_name: %s, baud_rate:%d, model_number:%d,mode: %s,p_gain:%d,i_gain:%d",
               i, id, dxl_name.c_str(), port_name[i].c_str(),baud_rate,model_number,mode.c_str(),p_gain,i_gain);
